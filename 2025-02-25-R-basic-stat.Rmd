...
If any of the ANOVA assumptions (independence observations, equal variance, normal distribution) are not met: non-parametric alternative is the Kruskal-Wallis (`kruskal_test` can be used for this)
...
Now standardize glucose and find the linear model again.
...
# Split Petal Length into 3 categories using equal frequency approach
# Using the cut function with 3 intervals: "Short", "Medium", and "Long"
iris$PetalLengthCategory <- cut(iris$Petal.Length, 
                                breaks = c(-Inf, 2.5, 4.5, Inf),  # Adjusted breakpoints
                                labels = c("Short", "Medium", "Long"))

# Create a contingency table: Species vs Petal Length Category
table_iris <- table(iris$Species, iris$PetalLengthCategory)
...
chi_result <- chisq.test(table_iris)
