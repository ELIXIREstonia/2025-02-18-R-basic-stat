...
Theory: \<TODO! add link to Zenodo slides here\>

Let's now do some hypothesis testing in R!

**One-sample t-test (intake dataset)**

i.e. "I have data about A, is there a significant difference in feature X based on the mean (or other metric) of the population of A?"
...
As we are comparing multiple groups against the same hypothesis, multiple testing correction should be undertaken.

```{r}
# TODO: using `p.adjust()` adjust the double "p_values" using different methods
##########################################################
# add your code here
...
Theory: \<TODO: add link to slides here\>
...
Perform the Mann-Whitney U Test (hint: `wilcox_test()`) to check for the difference in ozone levels in May and July.
